...
class WakeWordDetector:
    ...
    def monitor_for_interrupt(self, stop_event, interrupt_event) -> None:
        """Monitor for wake word and set *interrupt_event* when detected.

        Designed to run in a background thread.  Returns when *stop_event*
        or *interrupt_event* is set, or the wake word fires.
        """
        if self.engine != "openwakeword" or not self._model:
            # Fallback: sleep-poll until told to stop.
            while not stop_event.is_set() and not interrupt_event.is_set():
                time.sleep(0.1)
            return

        self._model.reset()
        self._last_trigger = 0.0  # clear holdoff for fresh monitoring
        ...
        try:
            with sd.InputStream(
                ...
                while not stop_event.is_set() and not interrupt_event.is_set():
                    ...
                    predictions = self._model.predict(samples)
                    if self._is_triggered(predictions):
                        logging.info("Interrupt: wake word detected during processing")
                        interrupt_event.set()
                        return
        except Exception:
            logging.exception("Wake word interrupt monitor error")
